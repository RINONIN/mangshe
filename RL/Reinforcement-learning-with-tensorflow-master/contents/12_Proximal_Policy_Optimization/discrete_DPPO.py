...
class PPONet(object):
    def __init__(self):
        ...
        lc = tf.compat.v1.layers.dense(self.tfs, 200, tf.compat.v1.nn.relu, kernel_initializer=w_init, name='lc')
        self.v = tf.compat.v1.layers.dense(lc, 1)
        ...
        self.tfa = tf.compat.v1.placeholder(tf.compat.v1.int32, [None, ], 'action')
        self.tfadv = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, 1], 'advantage')

        a_indices = tf.compat.v1.stack(
            [tf.compat.v1.range(tf.compat.v1.shape(self.tfa)[0], dtype=tf.compat.v1.int32), self.tfa], axis=1)
        pi_prob = tf.compat.v1.gather_nd(params=self.pi, indices=a_indices)  # shape=(None, )
        oldpi_prob = tf.compat.v1.gather_nd(params=oldpi, indices=a_indices)  # shape=(None, )
    ...
    def _build_anet(self, name, trainable):
        with tf.compat.v1.variable_scope(name):
            l_a = tf.compat.v1.layers.dense(self.tfs, 200, tf.compat.v1.nn.relu, trainable=trainable)
            a_prob = tf.compat.v1.layers.dense(l_a, A_DIM, tf.compat.v1.nn.softmax, trainable=trainable)
