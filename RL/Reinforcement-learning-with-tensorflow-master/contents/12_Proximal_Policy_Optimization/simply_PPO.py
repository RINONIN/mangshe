...
class PPO(object):
    ...
    def __init__(self):
        ...
        with tf.compat.v1.variable_scope('sample_action'):
            self.sample_op = tf.compat.v1.squeeze(pi.sample(1), axis=0)  # choosing action
        ...
        with tf.compat.v1.variable_scope('loss'):
            ...
            if METHOD['name'] == 'kl_pen':
                self.tflam = tf.compat.v1.placeholder(tf.compat.v1.float32, None, 'lambda')
                kl = tf.compat.v1.distributions.kl_divergence(oldpi, pi)
                self.kl_mean = tf.compat.v1.reduce_mean(kl)
                self.aloss = -(tf.compat.v1.reduce_mean(surr - self.tflam * kl))
        ...
        tf.compat.v1.summary.FileWriter("log/", self.sess.graph)
    ...
    def _build_anet(self, name, trainable):
        with tf.compat.v1.variable_scope(name):
            l1 = tf.compat.v1.layers.dense(self.tfs, 100, tf.compat.v1.nn.relu, trainable=trainable)
