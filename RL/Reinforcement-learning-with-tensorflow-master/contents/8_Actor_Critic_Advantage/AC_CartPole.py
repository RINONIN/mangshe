...
class Actor(object):
    def __init__(self, sess, n_features, n_actions, lr=0.001):
        ...
        with tf.compat.v1.variable_scope('exp_v'):
            log_prob = tf.compat.v1.log(self.acts_prob[0, self.a])  # log 动作概率
            self.exp_v = tf.compat.v1.reduce_mean(log_prob * self.td_error)   # log 概率 * TD 方向

        with tf.compat.v1.variable_scope('train'):
            # 因为我们想不断增加这个 exp_v (动作带来的额外价值),
            # 所以我们用过 minimize(-exp_v) 的方式达到
            # maximize(exp_v) 的目的
            self.train_op = tf.compat.v1.train.AdamOptimizer(lr).minimize(
...
for i_episode in range(MAX_EPISODE):
    ...
    while True:
        ...
        td_error = critic.learn(s, r, s_)  # gradient = grad[r + gamma * V(s_) - V(s)]  Critic 学习
        actor.learn(s, a, td_error)  # true_gradient = grad[logPi(s,a) * td_error] # Actor 学习
