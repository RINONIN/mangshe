...
class ACNet(object):
    def __init__(self, scope, globalAC=None):
        ...
        if scope == GLOBAL_NET_SCOPE:  # get global network
            ...
            with tf.compat.v1.variable_scope(scope):
                self.s = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, N_S], 'S')
                self.a_his = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, N_A], 'A')
                ...
                with tf.compat.v1.name_scope('choose_a'):  # use local params to choose action
                    self.A = tf.compat.v1.clip_by_value(tf.compat.v1.squeeze(normal_dist.sample(1), axis=[0, 1]),
                                                        A_BOUND[0], A_BOUND[1])
    ...
    def _build_net(self, scope):
        ...
        with tf.compat.v1.variable_scope('actor'):  # state representation is based on critic
            l_a = tf.compat.v1.layers.dense(cell_out, 80, tf.compat.v1.nn.relu6, kernel_initializer=w_init, name='la')
            mu = tf.compat.v1.layers.dense(l_a, N_A, tf.compat.v1.nn.tanh, kernel_initializer=w_init, name='mu')
            sigma = tf.compat.v1.layers.dense(l_a, N_A, tf.compat.v1.nn.softplus, kernel_initializer=w_init,
                                              name='sigma')
